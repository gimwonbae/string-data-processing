...
func match(path string, dirPath string) {
	var b bytes.Buffer
	wUnit, _ := os.OpenFile(filepath.Join(`..`, `w_unit`), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	woUnit, _ := os.OpenFile(filepath.Join(`..`, `wo_unit`), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	...
	for _, line := range lines {
		...
		if len(lineSplit) == 1 {
			...
			if end == -1 {
				...
				if matched {
					wUnit.WriteString(b.String())
				} else {
					woUnit.WriteString(b.String())
				}
				continue
			} else {
...
func woUnit(checkInput string, orgInput string) {
	var b bytes.Buffer
	checkList := toSlice(filepath.Join(`..`, checkInput))
	orgList := toSlice(filepath.Join(`..`, orgInput))

	match, _ := os.OpenFile(filepath.Join(`..`, `match_wo_unit`), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	miss, _ := os.OpenFile(filepath.Join(`..`, `miss_wo_unit`), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
...
func wUnit(checkInput string, orgInput string) {
	var b bytes.Buffer
	checkList := toSlice(filepath.Join(`..`, checkInput))
	orgList := toSlice(filepath.Join(`..`, orgInput))

	match, _ := os.OpenFile(filepath.Join(`..`, `match_w_unit`), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	miss, _ := os.OpenFile(filepath.Join(`..`, `miss_w_unit`), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
